...
void neutron::integrate(const double time, const double dt, const vector<double> &params,
                        vector<double> &tOut, vector<double> &xOut, vector<double> &yOut, 
			vector<double> &zOut)
{
    int t = 0;
    tOut.push_back(0);
...
// u[0] = Re(a), u[1] = Im(a), u[2] = Re(b), u(3) = Im(b)
{
    ...
    dudt[0] = 0.5 * (params[1] * u[1] + params[2] * cos(x) * u[3]) 
		- params[4] / 2 * params[2] * u[2] * sin(x);
    dudt[1] = 0.5 * (-params[1] * u[0] - params[2] * cos(x) * u[2]) 
		- params[4] / 2 * params[2] * u[3] * sin(x);
    dudt[2] = 0.5 * (-params[1] * u[3] + params[2] * cos(x) * u[1]) 
		+ params[4] / 2 * params[2] * u[0] * sin(x);
    dudt[3] = 0.5 * (params[1] * u[2] - params[2] * cos(x) * u[0]) 
		+ params[4] / 2 * params[2] * u[1] * sin(x);
    return dudt;
}
